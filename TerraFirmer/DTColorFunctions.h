...
/**
 ...
 @return A UIColor object generated from the hexadecimal color string with alpha 1.0.
 */
DTColor * _Nullable DTColorCreateWithHexString(NSString *hexString);
...
/**
 ...
 If a color begins with a `#` we know that it is a hexadecimal color and send it to colorWithHexString:. If the string is an `rgba()` color declaration the comma delimited r, g, b, and a values are made into percentages and then made into a DTColor which is returned. If the string is an `rgb()` color declaration the same process happens except with an alpha of 1.0.
 ...
 @return A UIColor object representing the name parameter as numeric values declared by the HTML and CSS specifications, a `rgba()` color, or a `rgb()` color.
 */
DTColor * _Nullable DTColorCreateWithHTMLName(NSString *name);
...
/**
 ...
 @return A CSS hexadecimal NSString specifying this DTColor.
 */
NSString * _Nullable DTHexStringFromDTColor(DTColor *color);
